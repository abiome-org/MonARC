...
| Initial Prior: p(T_0) = Uniform(SO(3) x R^3) across target region           |
| Input: Single camera frame I_t + Geo-indexed landmark field M               |
| Process: Coarse Global Retrieval (MegaLoc/n-gram) -> Constellation Match   |
...
- The initial pose distribution \( p(T_0) \) is uniform across the designated mission area.
- The system must retrieve candidate spatial tiles using global visual descriptors (MegaLoc-class or code n-gram hashes) and resolve ambiguous global matches using local metric constellations.
...
- The system queries a spatially constrained S2 map shard corresponding to the local bounding volume.
