...
University-1652 is the first implemented public-UAV loader (`monarc.data.uav_benchmarks.University1652`). The layout is a documented ImageFolder of building IDs (`train/drone/<id>`, `train/satellite/<id>`, `test/query_drone/<id>`, …). Download is optional (`--download` with `--download-url` / `MONARC_U1652_URL` to a licensed zip/tar). The dataset is request-gated; the loader does not scrape Google Drive. Unit tests use a synthetic JPEG fixture and never fetch. Street-view folders exist on the full dataset and are not ingested into the aerial codebook (see [`non-goals.md`](./non-goals.md) §2.5).
