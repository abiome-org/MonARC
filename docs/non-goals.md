...
|  - v1: one corridor + public benches            - CONUS raster factory / dense fp16 field |
+---------------------------------------------------------------------------------------+
```
...
- **Constraint**: If an area has not been ingested into the inverted metric index (v1: the designated corridor), MonARC reports indeterminate global uncertainty rather than emitting heuristic guesses.
...
- **Strategy**: The map is synthesized from pre-existing open federal geodata. v1 uses NAIP visualization COGs (~0.6 m, one vintage) and corridor 3DEP already-COG DSM, not a physical survey and not a CONUS ingest.
...
- **Boundary**: v1 must not ingest full-CONUS NAIP+3DEP, `naip-source` uncompressed archives, all historical NAIP years, CONUS 1 m lidar point clouds, Sentinel-2 / international coverage, or a dense continental fp16 / Zarr feature field. v1 must not egress-copy rasters to another cloud, train a new foundation backbone, require a custom flight-log campaign, or put a global map on the aircraft.
- **Law**: [`docs/cost.md`](./cost.md). Coverage is a corridor (default Jefferson County / Colorado Front Range). Planning envelope: under $2,000 cloud+data, preferably a few hundred dollars at county scale — planning envelopes, not invoices.
